...
@section('content')
    ...
    <div class="flex justify-between items-center mb-6">
        ...
        <div class="flex items-center gap-4">
            <!-- REPORT FORM -->
            <form action="{{ route('SuperAdmin.reporte.excel') }}" method="GET" class="flex items-center gap-2">
                <select name="mes"
                ...
                <button type="submit"
                    class="bg-green-600 hover:bg-green-700 text-white font-bold py-1 px-3 rounded text-sm flex items-center gap-1 transition transform hover:scale-105">
    ...
    <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
        ...
        <div
            ...
            <div class="bg-gray-50 px-4 py-3 border-b flex justify-between items-center">
                ...
                <button onclick="closeCreateModal()" class="text-gray-400 hover:text-gray-500 focus:outline-none">
            ...
            <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                ...
                <div class="mb-6 border-b border-gray-200">
                    <nav class="-mb-px flex space-x-8" aria-label="Tabs">
                        <button onclick="switchTab('empresa')" id="tab-empresa"
                            class="border-indigo-500 text-indigo-600 whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm w-1/2 text-center transition-colors hover:text-indigo-800">
                            1. Datos de la Empresa
                        </button>
                        <button onclick="switchTab('admin')" id="tab-admin"
                            class="border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300 whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm w-1/2 text-center transition-colors">
                ...
                <!-- FORM ADMIN -->
                <div id="content-admin" class="hidden space-y-4">
                    <form action="{{ route('administradores.store') }}" method="POST" enctype="multipart/form-data"
                        ...
                        <div class="col-span-2 flex justify-end mt-4">
                            <button type="submit"
                                class="bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-2 px-6 rounded-lg transition transform hover:scale-105">
                                Crear Administrador
                            </button>
                        </div>
                    </form>
...
<script>
    ...
    function switchTab(tab) {
        // Reset classes
        document.getElementById('tab-empresa').className = "border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300 whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm w-1/2 text-center transition-colors";
        document.getElementById('tab-admin').className = "border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300 whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm w-1/2 text-center transition-colors";

        document.getElementById('content-empresa').classList.add('hidden');
        document.getElementById('content-admin').classList.add('hidden');

        // Activate selected
        document.getElementById('tab-' + tab).className = "border-indigo-500 text-indigo-600 whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm w-1/2 text-center transition-colors hover:text-indigo-800";
        ...
        fetch(`/empresa/buscar/${nit}`)
            ...
            .then(data => {
                if (data && Object.keys(data).length > 0) {
                    ...
                    // Not found logic - keep unlocked and potentially clear if user wants partial match cleared
                    // But usually we just let them type if ID is new. 

                    // HOWEVER, if input IS cleared (length < 3 handled above), we should ensure unlocked.
                    // The 'else' here triggers if ID > 3 but not found in DB -> New Company -> Ensure Unlocked
                    unlock('create_nombre');
                    unlock('create_representante');
                    unlock('create_telefono');
                    unlock('create_correo');
