                    ...
                    <form action="{{ route('empresas.store') }}" method="POST" id="formEmpresa"
                        ...
                        <div class="col-span-2">
                            ...
                            <select name="id_tipo_licencia"
                                ...
                                <option value="{{ $tipo->id_tipo_licencia }}">{{ $tipo->nombre_licencia }} — ${{ number_format($tipo->precio, 0, ',', '.') }}</option>
                                @endforeach
                            </select>
                        </div>
...
<div id="adminModal" class="fixed inset-0 z-[9999] hidden overflow-y-auto" aria-modal="true">
    <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
        ...
        <div
            ...
            <!-- HEADER MODAL -->
            <div class="bg-gray-50 px-4 py-3 border-b flex justify-between items-center">
                <h3 class="text-lg leading-6 font-medium text-gray-900">Asignar Administrador</h3>
                <button onclick="closeAdminModal()" class="text-gray-400 hover:text-gray-500 focus:outline-none">
                    <svg class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M6 18L18 6M6 6l12 12" />
                    </svg>
            ...
            <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                ...
                <div id="content-admin" class="space-y-4">
                    <form action="{{ route('administradores.store') }}" method="POST" enctype="multipart/form-data" id="formAdmin"
                        class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        @csrf
...
<div id="editModal" class="fixed inset-0 z-[9999] hidden overflow-y-auto" aria-modal="true">
    <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
        ...
        <div class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-4xl w-full">
            ...
            <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                <form id="formEditEmpresa" action="" method="POST"
                    class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    @csrf
                    @method('PUT')
