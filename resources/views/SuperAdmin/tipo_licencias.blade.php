...
<div class="max-w-6xl mx-auto bg-white shadow-2xl shadow-slate-200/50 rounded-[2.5rem] border border-slate-100 overflow-hidden">

    {{-- HEADER DE LA SECCIÓN --}}
    <div class="p-8 sm:p-10 border-b border-slate-50 bg-gradient-to-r from-slate-50/50 to-white flex flex-col sm:flex-row justify-between items-center gap-6">
        <div>
            <div class="flex items-center gap-3 mb-2">
                <div class="p-2 bg-emerald-100 rounded-lg text-[#006b58]">
                    <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M9 5h12M9 12h12M9 19h12M5 5h.01M5 12h.01M5 19h.01" />
                    </svg>
                </div>
                <h2 class="text-3xl font-black text-green-800 tracking-tighter">Gestión De <span class="text-transparent bg-clip-text bg-gradient-to-r from-emerald-600 to-green-500">Planes</span></h2>
            </div>
        ...
        <a href="{{ route('licencias.create') }}"
            class="group flex items-center gap-3 bg-gradient-to-r from-[#34d399] via-[#22c55e] to-[#16a34a] hover:from-[#22c55e] hover:via-[#16a34a] hover:to-[#15803d] text-white px-8 py-4 rounded-2xl transition-all duration-300 transform active:scale-95 font-black uppercase tracking-widest text-xs">
    ...
    {{-- ALERTAS (Toast Style) --}}
    @if(session('success'))
    <div class="auto-dismiss fixed bottom-8 right-8 bg-emerald-600 text-white px-6 py-4 rounded-[1.5rem] shadow-2xl shadow-emerald-900/20 animate-fade-in-up z-50 flex items-center gap-3 border border-emerald-400/20">
    ...
    @if(session('error'))
    <div class="auto-dismiss fixed bottom-8 right-8 bg-rose-600 text-white px-6 py-5 rounded-[1.5rem] shadow-2xl shadow-rose-900/20 animate-fade-in-up z-50 max-w-sm flex items-start gap-3 border border-rose-400/20">
    ...
    <div class="overflow-x-auto">
        <table class="w-full text-left">
            <thead class="bg-slate-50/80 text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] border-b border-slate-100">
                <tr>
                ...
                <tr class="hover:bg-emerald-50/30 transition-all group">
                    ...
                    <td class="px-10 py-6">
                        <div class="flex items-center gap-3">
                            <span class="text-base font-black text-slate-700 group-hover:text-[#006b58] transition-colors">
                                {{ $licencia->nombre_licencia }}
                            </span>
                            @if($enUso)
                            <span class="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg text-[10px] font-black uppercase bg-emerald-100 text-emerald-700 border border-emerald-200">
                    ...
                    <td class="px-10 py-6">
                        <div class="flex justify-center items-center gap-3">
                            ...
                            <div class="relative group/tooltip">
                                ...
                                {{-- Tooltip Pro --}}
                                <div class="absolute bottom-full left-1/2 -translate-x-1/2 mb-3 w-56 hidden group-hover/tooltip:block z-20 animate-fade-in">
                                    <div class="bg-slate-800 text-white text-[10px] font-bold rounded-xl py-3 px-4 text-center shadow-2xl leading-relaxed">
