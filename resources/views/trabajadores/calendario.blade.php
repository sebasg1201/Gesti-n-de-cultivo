        ...
        if (dailyEvents.length === 0) {
            ...
            dailyEvents.forEach(ev => {
                ...
                // 2. FASES DE CULTIVO
                    ...
                    const colorFase = isPerdida ? 'red' : (isRealizado ? 'emerald' : 'blue');
                    const iconColor = isPerdida ? '#ef4444' : (isRealizado ? '#10b981' : '#3b82f6');
                    
                    listEl.insertAdjacentHTML('beforeend', `
                        <div class="bg-${colorFase}-50 border border-${colorFase}-100 p-6 rounded-[2rem] space-y-4 mb-4 ${isRealizado ? 'shadow-sm shadow-emerald-50/50' : ''}">
                            <div class="flex items-center gap-4">
                    ...
                    listEl.insertAdjacentHTML('beforeend', `
                        <div class="bg-${colorClass}-50 border border-${colorClass}-100 p-6 rounded-[2rem] space-y-4 mb-4 ${isRealizado ? 'shadow-sm shadow-emerald-50/50' : ''}">
                            <div class="flex items-center justify-between gap-4 w-full">
                                <div class="flex items-center gap-4">
                                    <div class="w-12 h-12 ${isRealizado ? 'bg-emerald-600 text-white' : 'bg-white text-gray-400'} rounded-2xl flex items-center justify-center border border-gray-100 shadow-lg" style="${isRealizado ? '' : (isPerdida ? 'color: #ef4444' : 'color: ' + ev.backgroundColor)}">
                                        ${isRealizado 
                                            ? '<svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M5 13l4 4L19 7"/></svg>'
                                            : (isPerdida 
                                                ? '<svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M6 18L18 6M6 6l12 12"/></svg>'
                                    ...
                                    <div>
                                        ...
                                        <p class="font-bold text-${colorClass}-900 ${isPerdida ? 'line-through opacity-70' : ''}">
