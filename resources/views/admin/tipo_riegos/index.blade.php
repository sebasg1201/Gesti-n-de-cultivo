        ...
        <div class="xl:col-span-1">
            <div class="bg-white rounded-3xl shadow-xl border border-blue-50 p-8 sticky top-8">
                <div class="flex items-center gap-4 mb-8">
                    ...
                    <div>
                        <h3 class="text-xl font-black text-emerald-950">Configurar Riego</h3>
                ...
                <div class="relative group mb-8">
                    <label class="block text-xs font-bold text-emerald-700 uppercase tracking-wider mb-3">Buscar sistema técnico</label>
                    <div class="relative">
                ...
                <form id="riegoForm" action="{{ route('tipo_riegos.store') }}" method="POST" class="space-y-6 hidden animate-in zoom-in-95 duration-200">
                    @csrf
                    <input type="hidden" name="id_catalogo" id="input_id_catalogo">

                    <div>
                        <label class="block text-xs font-bold text-emerald-700 uppercase tracking-wider mb-2">Nombre Personalizado</label>
                        <input type="text" name="tipo_riego" id="tipo_riego_name" required
                            class="w-full px-4 py-3 rounded-2xl border-emerald-100 focus:border-blue-500 focus:ring-0 bg-gray-50/50 text-sm font-medium">
                    </div>
                    ...
                    <div class="flex gap-3">
                        ...
                        <button type="submit" class="flex-1 bg-blue-600 hover:bg-blue-700 text-white font-black py-4 rounded-2xl shadow-xl shadow-blue-100 transition-all transform hover:-translate-y-1">
                            Habilitar Sistema
                        </button>
                    </div>
                </form>
        ...
        <div class="xl:col-span-2">
            <div class="bg-white rounded-3xl shadow-xl border border-emerald-50 overflow-hidden flex flex-col min-h-[600px]">
                <div class="p-8 border-b border-emerald-50 bg-gray-50/50 flex justify-between items-center">
                    <div>
                        <h3 class="text-2xl font-black text-emerald-950">Sistemas de Riego</h3>
                        <p class="text-xs font-medium text-emerald-600 mt-1">Configuración técnica activa</p>
                    </div>
                </div>
                ...
                <div class="flex-1 overflow-x-auto">
                    <table class="w-full text-left">
                        ...
                        <tbody class="divide-y divide-emerald-50/50">
                            ...
                            <tr class="hover:bg-blue-50/20 transition-all group">
                                <td class="px-8 py-6">
                                    <div class="flex items-center gap-4">
                                        ...
                                        <div>
                                            <span class="font-black text-emerald-950 block">{{ $riego->tipo_riego }}</span>
                                            <span class="text-[10px] font-bold text-blue-400 uppercase italic">Base: {{ $riego->catalogo->nombre ?? 'Manual' }}</span>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-8 py-6 text-center">
                                    <div class="inline-flex items-center gap-2 px-4 py-2 rounded-2xl {{ $riego->impacto_dias >= 0 ? 'bg-red-50 text-red-600 border-red-100' : 'bg-green-50 text-green-600 border-green-100' }} border font-black text-sm">
                                ...
                                <td class="px-8 py-6 text-right">
                                    <div class="flex items-center justify-end gap-3 opacity-0 group-hover:opacity-100 transition-all">
                                        ...
                                        <form action="{{ route('tipo_riegos.destroy', $riego->id_tipo_riego) }}" method="POST" class="inline">
                                            @csrf @method('DELETE')
                                            <button type="submit" onclick="return confirm('¿Eliminar este sistema?')" class="p-3 bg-red-50 text-red-600 rounded-2xl hover:bg-red-100 shadow-sm transition-all">
                                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            @empty
                            <tr>
                                <td colspan="4" class="px-8 py-32 text-center text-emerald-300 font-bold italic">No hay sistemas registrados.</td>
                            </tr>
                            @endforelse
                        </tbody>
                    </table>
