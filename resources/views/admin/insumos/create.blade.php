...
@section('content')
    <div class="max-w-3xl mx-auto space-y-6">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 sm:p-8">
            <div class="mb-6">
                <h2 class="text-2xl font-bold text-gray-800">Registrar Nuevo Insumo</h2>
                <p class="text-gray-500 text-sm mt-1">Ingrese los datos del insumo para registrarlo en el sistema.</p>
            </div>
            ...
            <form action="{{ route('admin.insumos.store') }}" method="POST" class="space-y-5">
                @csrf
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <select name="id_tipo_insumo"
                            ...
                            @foreach($tiposInsumo as $tipo)
                                <option value="{{ $tipo->id_tipo_insumo }}" {{ old('id_tipo_insumo') == $tipo->id_tipo_insumo ? 'selected' : '' }}>
                                    {{ $tipo->nombre_insumo }}
                                </option>
                            @endforeach
                        </select>
                        @error('id_tipo_insumo') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                        @if($tiposInsumo->isEmpty())
                            <p class="text-amber-600 text-xs mt-1 italic">
                                No hay tipos de insumo configurados. <a href="{{ route('tipo_insumos.index') }}"
                                    class="underline text-emerald-600 hover:text-emerald-700">Ir al catálogo</a>.
                            </p>
                        @endif
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="Calidad" value="{{ old('Calidad') }}"
                    ...
                    <div>
                        ...
                        <input type="number" name="cantidad_stock" value="{{ old('cantidad_stock', 0) }}" min="0"
                    ...
                    <div class="sm:col-span-2">
                        ...
                        <textarea name="descripcion" rows="3" placeholder="Descripción adicional del insumo..."
                            class="w-full px-4 py-2 border border-gray-200 rounded-lg focus:outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 resize-none">{{ old('descripcion') }}</textarea>
                    </div>
                </div>
